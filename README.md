...
1. **结构**：必须严格遵循**七段式 MDX 结构**
   - 一、案发
   - 二、人物（使用 `<PersonCard>`）
   - 三、证词（使用 `<TestimonyBlock>`）
   - 四、物证（使用 `<EvidenceList>` + `export const evidence`）
   - 五、推理问题（使用 `<DeductionQuestions>`，通常 5–6 个问题）
   - 六、你的推理（使用 `<DetectiveNotes>`）
   - 七、揭晓真相（使用 `<RevealAnswer>`，包含「真相」「关键矛盾」「伏笔解析」）

2. **文风**：优雅、克制、文学性中文，强调**公平本格**（fair play）。伏笔解析需详细（通常 5–6 点）。

3. **灵感来源**：新案卷必须从世界著名侦探小说中汲取诡计灵感，包括但不限于：
   - 日本：松本清张、江户川乱步、绫辻行人、贵志祐介
   - 欧美：阿加莎·克里斯蒂、埃勒里·奎因、约翰·狄克森·卡尔、阿瑟·柯南·道尔
   - 其他：安东尼·霍洛维茨、高野和明等

4. **时间戳规则**：
   - 新增案卷/线索的 `publishedAt` 必须使用**当天日期**
   - 历史内容时间戳应分散在 4 月、5 月、6 月不同日期
   - 列表页按 `publishedAt` 倒序排列（最新内容自动排在最前）
...
1. 创建 `.mdx`（案卷）或 `.md`（线索）文件
2. 写入当天日期的 `publishedAt`
3. 运行 `pnpm og:export` 生成分享图
4. 提交并更新 README 更新日志


> **注意**：从 2026-06-19 起，所有新创建的案卷和线索，`publishedAt` 字段必须使用**创建当天的日期**。


### 2026-06-19 — 新增 5 篇案卷 + 10 篇线索（#030–#034）
...
**线索（新增 10 篇，order 25–34）**：机械延迟、桥牌诡计、文字游戏、隐藏通道、虚假不在场、心理误导、物证栽赃、叙述者不可靠、公平本格边界、现代本格趋势。

所有新内容 `publishedAt` 均为 2026-06-19，并已运行 `pnpm og:export`。
