...
2. **`caseId`**：三位数字字符串（`"001"`…），按 `publishedAt` 从旧到新**连续编号**，**禁止跳号、乱序**。当前已用到 `"188"`，下一篇从 `"189"` 起。
...
| 2026-06-28 | 136–140 | 5 | — | 仪器/机械物证批次（天文台星轨/潜水电脑表/落地钟打点/马拉松计时芯片/殡仪馆火化）；每案 3 道选择题 |
...
| 2026-07-15 | 156–160 | 5 | — | 行业记录仪批次（酒庄阀门计数器/缫丝厂张力纸带/透析血泵转数/跳伞AAD高度计/古籍修复紫外灯）；每案 3 道选择题 |
| 2026-07-21 | 161–165 | 5 | — | 行业记录仪批次二（电台播出日志/紫砂窑温曲线/消防气瓶压力记录/马术心率项圈/盾构推力记录）；每案 3 道选择题 |
| 2026-07-26 | 166–170 | 5 | — | 行业监测/日志批次三（血库温度记录仪/布草 RFID 二次洗涤/变电站 SCADA 尖峰/洗印机槽位计时/污水厂 ORP 探头）；每案 3 道选择题 |
| 2026-07-26 | 线索 96–100 | 5 | — | 配套线索（冷链温度记录仪/RFID 二次循环/SCADA 与天气借口/工序计时跳步/探头旁路走平） |
| 2026-07-28 | 171–173 | 3 | — | 行业记录仪批次四（过山车锁扣传感器/赛车ECU黑匣子/数据中心温控日志）；每案 3 道选择题 |
| 2026-08-04 | 174–178 | 5 | — | 行业记录仪批次五（高尔夫灌溉记录/造纸烘缸温控/巧克力调温曲线/手术麻醉记录/帆船GPS航迹）；每案 3 道选择题 |
| 2026-08-11 | 179–180 | 2 | — | 行业记录仪批次六（影院放映服务器日志偷票房/电竞反作弊日志代打）；每案 3 道选择题 |
| 2026-08-18 | 181–183 | 3 | — | 行业记录仪批次七（果汁厂巴氏杀菌记录/汽修厂OBD诊断记录/温泉水疗馆余氯监测）；每案 3 道选择题 |
| 2026-09-23 | 184–188 | 5 | — | 行业记录仪批次八（网约车计价器日志/快递分拣扫描日志/影视渲染农场日志/直播电商后台数据/植保无人机飞控日志）；每案 3 道选择题 |
...
| 136 | 星图差了一帧 | `the-star-log-off-by-one-frame` | ★ |
| 137 | 没有上浮记录的潜水表 | `the-dive-computer-that-never-surfaced` | |
| 138 | 敲了十三下的钟 | `the-clock-that-chimed-thirteen` | ★ |
| 139 | 没有人跑的那一棒 | `the-relay-split-that-never-ran` | |
| 140 | 太轻的那只骨灰盒 | `the-urn-that-weighed-too-little` | ★ |
| 141 | 反扣的那只快挂 | `the-carabiner-clipped-backward` | |
| 142 | 夜班印错的那一令纸 | `the-night-run-with-the-wrong-ink` | ★ |
| 143 | 温室里那粒早开的花粉 | `the-greenhouse-pollen-clock` | |
| 144 | 多走了一趟的缆车 | `the-gondola-that-counted-one-more` | ★ |
| 145 | 芯片读出另一只狗 | `the-chip-that-named-another-dog` | |
| 146 | 纬纱梭多数了一拍 | `the-weaving-shuttle-stopped-on-the-wrong-count` | ★ |
| 147 | 半夜沉下去的蜂箱秤 | `the-hive-scale-dipped-at-midnight` | |
...
| 91 | 电子秤读数：重量也是时钟 | `electronic-scale-logs-as-clocks` |
| 92 | 织机梭位：机械计数改不了 | `loom-shuttle-counters-as-records` |
| 93 | 色带叠印：打字机记得每一分钟 | `typewriter-ribbon-overlap-tricks` |
| 94 | 清冰车路线：没压过的冰会留纹 | `ice-resurfacer-route-logs` |
| 95 | 点名册缺口：重复姓名在替谁站位 | `roll-call-manifest-gaps` |
| 96 | 冷链温度记录仪：门开了多久 | `cold-chain-temperature-loggers` |
| 97 | RFID 二次循环：洗一遍给账看 | `rfid-inventory-double-cycles` |
| 98 | SCADA 与天气借口：尖峰从哪来 | `scada-vs-weather-excuses` |
| 99 | 工序计时被跳过：少走的那一步 | `process-timer-skipped-steps` |
| 100 | 探头旁路与走平曲线：监测如何闭嘴 | `sensor-bypass-flatline-curves` |

新增线索：`order` 从 **101** 起，在 `src/content/clues/` 新建 `.md`。
...
### 2026-07-26 — 案卷扩充（166–170）

- 新增案卷 **5 篇**（caseId 166–170）：行业监测/日志批次三
  - 166 血库冰箱暖了十二分钟（中心血站·温度记录仪升温窗口 vs「门未开/停电」证词）
  - 167 芯片多洗了一次的那批床单（酒店洗衣房·RFID 二次全循环 vs 生产单只一轮）
  - 168 不是雷击的那次电压尖峰（变电站·SCADA 旁路合闸 vs 雷达远方雷击）
  - 169 跳过一道药浴的显影计时（胶片洗印厂·槽位停留 0 秒 vs 画格植入盗样）
  - 170 午夜走平的氧化还原电位（污水厂·ORP 走平 vs 加药泵电流，探头旁路）
- 每案 **3 道** frontmatter 选择题 + **5 题** `DeductionQuestions`；难度 4–5；featured：166、169
- `pnpm check` 0 errors；`pnpm og:export` + `pnpm verify:og` 通过（170 案全配图）
- 案卷总数 **165 → 170**；线索总数不变 **89**；下一篇 `caseId` **171**

### 2026-07-26 — 线索扩充（96–100）

- 新增线索 **5 篇**（order 96–100），与 166–170 行业监测批次配套：
  - 96 冷链温度记录仪：门开了多久
  - 97 RFID 二次循环：洗一遍给账看
  - 98 SCADA 与天气借口：尖峰从哪来
  - 99 工序计时被跳过：少走的那一步
  - 100 探头旁路与走平曲线：监测如何闭嘴
- 线索总数 **89 → 94**；一览表、批次记录、规模数字同步；下一线索 `order` **101**
...
- 备注：本批次原计划编号 166–168，写作期间另一会话已并发把 166–170 推送至 `main`，编号冲突后重新分配为 171–173
...
- 新增案卷 **5 篇**（caseId 146–150）：工业与日常物证批次
  - 146 纬纱梭多数了一拍（纺织厂·机械停梭位 vs 电子产量覆写）
  - 147 半夜沉下去的蜂箱秤（养蜂场·物联网秤骤降与账面差额）
  - 148 色带重叠的那一分钟（打字机修理店·色带叠印伪造遗嘱时间）
  - 149 四号道没清过的冰（溜冰馆·清冰车 GPS 跳过与旧冰纹）
  - 150 救生艇点名少了一名（邮轮·重复姓名掩盖缺员）
- 新增线索 **5 篇**（order 91–95），与案卷诡计配套
- 每案 **3 道** frontmatter 选择题 + **5 题** `DeductionQuestions`；难度 4–5（150 为 5）
- 案卷总数 **145 → 150**；线索总数 **84 → 89**；下一篇 `caseId` **151**

### 2026-06-29 — 案卷扩充（141–145）

- 新增案卷 **5 篇**（caseId 141–145）：场景/物证扩展批次
  - 141 反扣的那只快挂（攀岩馆·确保失效物证 + 镁粉掌纹/前台系统时间）
  - 142 夜班印错的那一令纸（印刷厂·偷印防伪票证，机器计数/墨辊温度揭穿停机）
  - 143 温室里那粒早开的花粉（植物园·昙类花粉夜间时钟锁定死亡时间）
  - 144 多走了一趟的缆车（滑雪场·缆车空载计数 + 电机日志/唯一钥匙）
  - 145 芯片读出另一只狗（宠物医院·倒卖管制药 + 火化焚尸，宠物芯片身份不符）
- 每案 **3 道** frontmatter 选择题 + **5 题** `DeductionQuestions`；难度 3–4
- `pnpm check` 0 errors；`og:export` + `verify:og` 通过（145 案全配图）
- 案卷总数 **140 → 145**；一览表、批次记录、规模数字、下一篇 `caseId`（146）同步
...
- 新增案卷 **5 篇**（caseId 136–140）：仪器/机械物证批次
  - 136 星图差了一帧（天文台·星轨天然时钟 vs 篡改日志）
  - 137 没有上浮记录的潜水表（潜水电脑表深度曲线揭穿伪造溺亡）
  - 138 敲了十三下的钟（古董钟表店·拨钟伪造时间线，多敲一下露馅）
  - 139 没有人跑的那一棒（马拉松·计时芯片随号码布替跑）
  - 140 太轻的那只骨灰盒（殡仪馆·骨灰重量与植入物批号揭穿遗体调换）
- 每案 **3 道** frontmatter 选择题 + **5 题** `DeductionQuestions`；难度 3–4
- `pnpm check` 0 errors；`pnpm og:export` + `verify:og` 通过（140 案全配图）
- 案卷总数 **135 → 140**；一览表、批次记录、顶部规模数字、下一篇 `caseId`（141）同步
